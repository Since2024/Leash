...
fn build_depth3_chain(svm: &mut LiteSVM, s: &Setup, merchant_ata: Pubkey) -> Chain {
    ...
    let root_capability = capability_pda(&root_owner.pubkey());
    expect_ok(issue(svm, s, &root_owner, 1_000, FAR_FUTURE, vec![merchant_ata]));

    let a_owner = Keypair::new();
    let a_capability = capability_pda(&a_owner.pubkey());
    expect_ok(attenuate(svm, s, &root_owner, root_capability, &a_owner, 500, FAR_FUTURE, vec![merchant_ata]));

    let b_owner = Keypair::new();
    let b_capability = capability_pda(&b_owner.pubkey());
    expect_ok(attenuate(svm, s, &a_owner, a_capability, &b_owner, 200, FAR_FUTURE, vec![merchant_ata]));

    let c_owner = Keypair::new();
    let c_capability = capability_pda(&c_owner.pubkey());
    expect_ok(attenuate(svm, s, &b_owner, b_capability, &c_owner, 50, FAR_FUTURE, vec![merchant_ata]));
...
fn depth3_spend_succeeds_when_nothing_revoked() {
    ...
    expect_ok(spend(&mut svm, &s, &chain.c_owner, &merchant_ata, 10));
...
fn depth3_spend_fails_when_immediate_parent_revoked() {
    ...
    expect_err(
        spend(&mut svm, &s, &chain.c_owner, &merchant_ata, 10),
...
fn attenuate_rejects_child_cap_exceeding_parent_remaining() {
    ...
    expect_ok(issue(&mut svm, &s, &parent_owner, 100, FAR_FUTURE, vec![merchant_ata]));

    // Exactly at the boundary: succeeds.
    let child_ok_owner = Keypair::new();
    expect_ok(attenuate(&mut svm, &s, &parent_owner, parent_capability, &child_ok_owner, 100, FAR_FUTURE, vec![merchant_ata]));
    ...
    expect_err(
        attenuate(&mut svm, &s, &parent_owner, parent_capability, &child_over_owner, 1, FAR_FUTURE, vec![merchant_ata]),
...
fn attenuate_rejects_depth_past_max_depth() {
    ...
    expect_err(
        attenuate(&mut svm, &s, &chain.c_owner, chain.c_capability, &too_deep_owner, 1, FAR_FUTURE, vec![merchant_ata]),
...
fn spend_respects_expiry_boundary() {
    ...
    expect_ok(issue(&mut svm, &s, &principal, 500, expiry, vec![merchant_ata]));

    // Before expiry: succeeds.
    expect_ok(spend(&mut svm, &s, &principal, &merchant_ata, 10));
    ...
    expect_err(
        spend(&mut svm, &s, &principal, &merchant_ata, 11),
